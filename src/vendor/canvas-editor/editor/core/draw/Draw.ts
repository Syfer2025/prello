...
export class Draw {
  ...
  // Ponto de extensão oficial: roda depois de computeRowList montar as linhas,
  // antes de devolvê-las. O Prelo usa isto para justificação por palavra e recuo
  // de 1ª linha SEM substituir o método (no lugar do antigo monkey-patch externo).
  private computeRowListHook:
    | ((rowList: IRow[], payload: IComputeRowListPayload) => void)
    | null = null
  ...
  public setComputeRowListHook(
    hook: ((rowList: IRow[], payload: IComputeRowListPayload) => void) | null
  ) {
    this.computeRowListHook = hook
  }
  ...
  public computeRowList(payload: IComputeRowListPayload) {
    ...
    // Extensão Prelo (justificação por palavra / recuo de 1ª linha) antes de
    // devolver as linhas. Sem hook registrado, é um no-op.
    this.computeRowListHook?.(rowList, payload)
    return rowList
  }
